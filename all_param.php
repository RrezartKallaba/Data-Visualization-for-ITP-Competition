...
<body>
    ...
    <nav id="myLinks" class="menu">
      <section class="manage">
        ...
        <li>
            ...
            <li style="border-bottom: 1px solid #709F31;border-radius: 25px;" class="active">
      ...
      <div class="sidebar" id="sidebar">
        <form method="GET" action="" class="date-form">
          <div class="row">
            ...
            <select id="sorting" name="sorting" class="date-input">
  ...
  $sorting = $_GET['sorting'] ?? 'months'; // Default to 'months'
  ...
  foreach ($csv as $row) {
    ...
    } elseif ($sorting == 'weeks') {
      $period = $timestamp->format('o-W'); // Year and ISO-8601 week number
    } else { // Default to 'months'
      $period = $timestamp->format('Y-m');
    }
    ...
    if (!isset($dataByPeriod[$period][$sensor])) {
      $dataByPeriod[$period][$sensor] = ['sum' => 0, 'count' => 0];
    }
  ...
  foreach ($dataByPeriod as $period => &$sensors) {
    foreach ($sensors as $sensor => $values) {
      $sensors[$sensor] = $values['sum'] / $values['count'];
    }
  }

  ksort($dataByPeriod);

  $json_data = json_encode($dataByPeriod);
  ?>
  ...
  <script>
    ...
    const colors = {
      'SOIL1': 'rgba(75, 192, 192, 1)',
      'US': 'rgba(255, 159, 64, 1)',
      'PRES': 'rgba(153, 102, 255, 1)',
      'HUM': 'rgba(54, 162, 235, 1)',
      'TC': 'rgba(255, 99, 132, 1)'
    };

    function createCombinedChart(canvasId, data, colors) {
      const labels = Object.keys(data);
      const datasets = [];

      const sensors = Object.keys(colors);
      sensors.forEach(sensor => {
        const sensorData = labels.map(label => data[label][sensor] || null);
        datasets.push({
          label: sensor,
          data: sensorData,
          borderColor: colors[sensor],
          backgroundColor: colors[sensor].replace('1)', '0.2)'),
        });
      });
        ...
        options: {
          ...
          plugins: {
            ...
            title: {
              ...
              font: {
                size: 14 // Adjust font size for chart title
              }
            }
          }
        }
    ...
    createCombinedChart('combinedChart', data, colors);
  </script>
